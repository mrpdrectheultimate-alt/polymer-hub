...
async function runFixAndUpgrade() {
  console.log("=== SPRINT 1B BATCH 1: UPGRADING ALL 5 PRIORITY LESSONS ===");

  for (const item of BATCH1_UPGRADES) {
    console.log(`\nUpgrading lesson for slug: ${item.slug}...`);
    ...
    if (fetchErr || !rows || rows.length === 0) {
      console.error(`  Could not find lesson for slug ${item.slug}:`, fetchErr?.message);
      continue;
    }
    ...
    if (updateErr) {
      ...
      console.log(`  Successfully upgraded lesson: "${item.title}" to 8-block launch-ready standard!`);
    }
  }

  console.log("\nSprint 1B Batch 1 upgrade completed 100% cleanly for ALL 5 PRIORITY LESSONS!");
}
